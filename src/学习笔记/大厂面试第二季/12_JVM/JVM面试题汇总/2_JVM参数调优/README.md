...
- XX参数（重点）
  ...
  - key-value类型
    - 公式：-XX:属性key=属性value
    - 不满意初始值，可以通过下列命令调整
    - case：例如：-XX:MetaspaceSize=21807104：查看Java元空间的值
...
```
8116 com.moxi.interview.study.GC.HelloGC
...
```shell
jps -l
jinfo -flag MetaspaceSize 13540
```
得到默认值：
```shell
...
- -Xss：设计单个线程栈的大小，一般默认为512K~1024K，等价于 -XX:ThreadStackSize
  - 使用jinfo -flag ThreadStackSize   会发现 -XX:ThreadStackSize = 0
  - **这个值的大小是取决于平台的**
  - Linux/x64: 1024KB
  - OS X:  1024KB
